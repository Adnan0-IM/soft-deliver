		...
		<>
			<header className="h-16 bg-card border-b border-border flex items-center justify-between px-4 md:px-6">
				<div className="flex items-center gap-3">
					<Button className="md:hidden" variant={"outline"} onClick={() => setOpen(true)}>
						<Menu />
					</Button>
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button variant="ghost" className="p-0 rounded-full h-9 w-9">
								<Avatar className="h-9 w-9">
									<AvatarImage src="/avatar-admin.png" alt="Driver Avatar" />
									<AvatarFallback>{(user?.name?.[0] ?? "D").toUpperCase()}</AvatarFallback>
								</Avatar>
							</Button>
						</DropdownMenuTrigger>
						<DropdownMenuContent align="end" className="min-w-56">
							<DropdownMenuLabel className="flex flex-col">
								...
								<span className="text-xs text-muted-foreground">{user?.email ?? "driver@example.com"}</span>
							</DropdownMenuLabel>
							<DropdownMenuSeparator />
							<DropdownMenuItem onClick={() => navigate("/driver/profile")}>Profile</DropdownMenuItem>
							<DropdownMenuItem onClick={() => navigate("/driver")}>Dashboard</DropdownMenuItem>
							<DropdownMenuSeparator />
							<DropdownMenuItem
								onClick={() => {
									logout()
